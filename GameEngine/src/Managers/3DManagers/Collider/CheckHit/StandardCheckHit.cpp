...
namespace REngine
{
	...
	namespace Collision
	{
		bool CheckHit_Sphere_Sphere(ColliderBase* colA, ColliderBase* colB, HitInfomation* info)
		{
			// コライダーを変換
			SphereCollider* col1 = static_cast<SphereCollider*>(colA);
			SphereCollider* col2 = static_cast<SphereCollider*>(colB);
		...
		bool CheckHit_Sphere_Capsule(ColliderBase* colA, ColliderBase* colB, HitInfomation* info)
		{
			// コライダーを変換
			SphereCollider* col1 = static_cast<SphereCollider*>(colA);
			CapsuleCollider* col2 = static_cast<CapsuleCollider*>(colB);
		...
		bool CheckHit_Sphere_Box(ColliderBase* colA, ColliderBase* colB, HitInfomation* info)
		{
			// コライダーを変換
			SphereCollider* col1 = static_cast<SphereCollider*>(colA);
			BoxCollider* col2 = static_cast<BoxCollider*>(colB);
		...
		bool CheckHit_Capsule_Capsule(ColliderBase* colA, ColliderBase* colB, HitInfomation* info)
		{
			// コライダーを変換
			CapsuleCollider* col1 = static_cast<CapsuleCollider*>(colA);
			CapsuleCollider* col2 = static_cast<CapsuleCollider*>(colB);
		...
		bool CheckHit_Capsule_Box(ColliderBase* colA, ColliderBase* colB, HitInfomation* info)
		{
			// コライダーを変換
			CapsuleCollider* col1 = static_cast<CapsuleCollider*>(colA);
			BoxCollider* col2 = static_cast<BoxCollider*>(colB);
		...
		bool CheckHit_Box_Box(ColliderBase* colA, ColliderBase* colB, HitInfomation* info)
		{
			// コライダーを変換
			BoxCollider* col1 = static_cast<BoxCollider*>(colA);
			BoxCollider* col2 = static_cast<BoxCollider*>(colB);

			// OBBの各軸3本×2, 軸同士の外積3本×3本 この15本の分離軸に対して離れているか確認

			// 各軸を取得
			SimpleMath::Vector3 naX = col1->GetXAxis(), naY = col1->GetYAxis(), naZ = col1->GetZAxis();
			SimpleMath::Vector3 nbX = col2->GetXAxis(), nbY = col2->GetYAxis(), nbZ = col2->GetZAxis();

			// 中心点を取得
			SimpleMath::Vector3 aCenter = col1->GetWorldCenterPos(), bCenter = col2->GetWorldCenterPos();
			SimpleMath::Vector3 interval = aCenter - bCenter;

			// サイズを取得
			SimpleMath::Vector3 aSize = col1->GetHalfSize(), bSize = col2->GetHalfSize();
